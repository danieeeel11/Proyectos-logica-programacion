...
public class AppEmpresa {
	...
	public static void main(String[] args) {
		...
		for(int i=0;i<esf.length;i++) {
			...
			else if(añosT>=2 && añosT<=3) {
				salarioTotal=(salario*0.05)+salario;
				
			}
			else if(añosT>=4 && añosT<=7) {
				salarioTotal=(salario*0.1)+salario;
			}
			else if(añosT>=8 && añosT<=15){
			...
			EmSalarioFijo emsf = new EmSalarioFijo(id,nombre,apellido,fechaIngreso,salario,añosT,salarioTotal);
		...
		for(int i=0; i<ec.length;i++) {
			...
			Fecha fechaIngreso =new Fecha(dia, mes, año);
			EmComision emc =new EmComision(id,nombre,apellido,fechaIngreso,300000,añosT,com,salarioTotal);
