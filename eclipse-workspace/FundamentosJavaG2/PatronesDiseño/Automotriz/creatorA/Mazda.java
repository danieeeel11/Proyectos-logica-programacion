...
public class Mazda extends AbstractFactory{
	public String marca;
	...
	public String createModelo() {
		...
		for (int i = 0; i < bMod.modelosMazda.size(); i++) {
			System.out.print("  " + (i+1) + ". " + bMod.modelosMazda.get(i) + "\n");
		}
		int number = sc.nextInt();
		while(number>bMod.modelosMazda.size()) {
			System.out.println("Digite una opcion de modelo valida");
			number = sc.nextInt();
		}
		String modelo = bMod.modelosMazda.get((number-1));
		return modelo;
	}
