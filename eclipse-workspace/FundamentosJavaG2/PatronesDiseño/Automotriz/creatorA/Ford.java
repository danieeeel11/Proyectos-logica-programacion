...
public class Ford extends AbstractFactory{
	public String marca;
	...
	public String createModelo() {
		...
		for (int i = 0; i < bMod.modelosFord.size(); i++) {
			System.out.print("  " + (i+1) + ". " + bMod.modelosFord.get(i) + "\n");
		}
		int number = sc.nextInt();
		while(number>bMod.modelosFord.size()) {
			System.out.println("Digite una opcion de modelo valida");
			number = sc.nextInt();
		}
		String modelo = bMod.modelosFord.get((number-1));
		return modelo;
	}
